...
* **一揽子股票**
  * 监管公式可按股票类别计算，但公司当前不允许以个股现货对冲股权衍生品，因此前端直接阻断该工具。
...
* **对冲有效性判定：** 提供一个判断"是否达成监管有效对冲（Delta匹配）"的标识，决定风险资本准备的计算标准是维持10% ~ 100%，还是降至1% ~ 5%。
  ...
  * 公司无海外业务凭证，客户端及对冲端境外市场交易直接阻断；公司当前禁止个股现货对冲，ETF可用但其是否宽基必须单独确认。
## 输出列表：
**第一层：现金流与资源消耗绝对值**
...
$$\text{RiskCap}_{\text{unhedged}} = \text{RiskCap}_{\text{client}} + \text{RiskCap}_{\text{hedge}}$$
  ...
  - ETF：权益类指数基金按 $\text{市值}\times5\%$，其他权益类基金按 $\text{市值}\times10\%$；股票才适用 8%/25%/50%/80%，且公司当前禁止个股现货对冲。
