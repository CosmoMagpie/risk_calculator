...
#
# 【代码架构：两大业务模块 + 一组计算引擎】
#   模块A - ClientContract（客户端）：场外衍生品合约，包括场外期权、收益互换、收益凭证
#   模块B - HedgeTrade（交易端）：用于对冲客户端风险的工具，包括ETF、股票、期货、期权等
#   计算引擎 - 一系列函数：输入 ClientContract + HedgeTrade → 输出各项风险指标变化
#
# 【Python 语法要点速览】
...
DEFAULT_CONFIG = {
    ...
    "firm": {
        ...
        "on_off_balance_total_asset_base": 300_000_000_000,  # 表内外资产总额（元），2025年12月31日表内总资产203_218_275_359.99
    }
}#数据来自之前业务的excel表格
...
#
# 【业务含义】ClientContract 代表股衍部与客户签订的一笔场外衍生品合约
...
class ClientContract:
    ...
    def get_client_cash_inflow(self) -> float:
        """
        计算客户端首日现金净流入（万元，正值=现金流入券商，负值=流出）
...
#   当券商与客户做了一笔衍生品交易（ClientContract）后，会在市场上建立反向头寸
...
class HedgeTrade:
    ...
    def get_trade_cash_outflow(self) -> float:
        ...
        【与 ClientContract.get_client_cash_inflow 对应】
    ...
    def get_hedge_delta_amount(self) -> Optional[float]:
        ...
        【设计思路】与 ClientContract.get_client_delta_amount 镜像对应
          客户端Delta + 对冲端Delta → 判断是否构成有效对冲
        
        【注意】此处存在一个潜在bug：
          DEFAULT_CONFIG["trade"]["onsite_option_delta"] 这个key在配置中不存在，
          配置中的key是 "atm_option_delta"，运行时可能会抛出 KeyError
        """
        ...
        if self.tool_type == "onsite_option":
            ...
            if self.option_delta is not None:
                return self.option_delta * self.notional
            # ⚠️ BUG: "onsite_option_delta" 在 DEFAULT_CONFIG["trade"] 中不存在
...
# ===================== 核心计算函数 =====================
# 【整体数据流】
#   ClientContract + [HedgeTrade, ...] 
#     → get_underlying_price() + calculate_correlation() → is_effective_hedge()
...
def get_underlying_price(underlying_code: str) -> Dict[str, List[float]]:
    ...
    【当前状态】使用硬编码的示例数据（各仅10个数据点），仅用于原型验证
    【后续方向】应替换为从数据库/API获取真实历史价格（如Wind、Bloomberg）
    
    【Python语法】List[float] 表示浮点数列表；
                  Dict[str, List[float]] 表示键是字符串、值是浮点数列表的字典
    """
    # 示例数据，实际使用时替换为真实数据源
    sample_data = {
    ...
    # 【设计思路】对未知代码，用hash值做随机种子生成模拟价格，确保相同代码每次返回一致
    if underlying_code not in sample_data:
        import random
        random.seed(hash(underlying_code) % 10000)         # 固定随机种子保证可重复
        prices = [100.0 + random.uniform(-20, 20) for _ in range(10)]  # 列表推导式生成10个价格
...
def calculate_correlation(client_code: str, client_prices: Dict[str, List[float]],
    ...
    【Python语法】pd.Series 将列表转为Pandas序列，.corr() 直接计算相关系数
    """
    # 从字典中提取价格列表，转为Pandas Series
    ...
    client_aligned = client_series.iloc[:min_len]   # .iloc 是按位置索引切片
...
def is_effective_hedge(client: ClientContract, hedge_list: List[HedgeTrade]) -> bool:
    ...
    【Python语法】
      - any(cond for x in list)：列表中有任一元素满足条件即返回True
      - all_positions.append((a, b))：往列表追加元组（tuple），元组是不可变的轻量容器
      - sum(abs(delta) for dir_, delta in lst if dir_ == "long")：生成器表达式，惰性计算
      - min(a,b)/max(a,b)：计算较小/较大值之比，避免判断孰大孰小
    """
    # ========== 条件1：判断是否同一标的或高相关性 ==========
    client_underlying_code = client.get_underlying_code()
    if client_underlying_code is None:
        return False  # 客户端没有标的代码，无法判断
    
    # 提取所有对冲工具的代码（过滤掉 None）
    ...
    # ========== 条件2：Delta对冲比例检查（80%-125%） ==========
    # 将客户端与所有对冲工具的(direction, delta_amount)汇总到同一列表
    all_positions = []
...
def calc_market_risk_reserve(client: ClientContract, hedge_list: List[HedgeTrade], is_hedge_effective: bool) -> float:
    ...
    #    但 ClientContract 中实际用的是 "call_option"/"put_option"/"equity_swap"/"income_certificate"
    ...
    if client.contract_type == "option":
        if client.direction == "buy":
            ...
            if client.stress_loss is not None:
                ...
                # ⚠️ BUG: client 没有 delta_amount 属性，应该是 client.get_client_delta_amount()
        ...
        exposure = client.notional * 0.10  # 互换风险敞口 = 名义本金 × 10%
        ...
        if client.expected_yield > 0.05:
            # 收益凭证预期收益率 > 5% 才有市场风险
            reserve += client.notional * 0.05 * RATES["futures_swap"]  # ⚠️ BUG: key不存在
...
def calc_lcr_impact(client: ClientContract, hedge_list: List[HedgeTrade]) -> Dict[str, float]:
    ...
    【Python语法】字典返回多个值，用 dict["key"] 访问
    """
    ...
    if client.term_days <= 30:
        if client.contract_type == "income_note":
            # 收益凭证30日内到期：需偿还募集资金
            cof_change += client.funds_raised or client.notional  # X or Y：取第一个非None/非零值
        elif client.contract_type == "swap":
            # 互换30日内到期：需返还客户保证金
...
def analyze_contract(client: ClientContract, hedge_list: List[HedgeTrade]) -> Dict[str, Any]:
    ...
    【数据流】
      输入：ClientContract（客户端合约）+ [HedgeTrade, ...]（对冲工具列表）
    ...
    【Python语法】Dict[str, Any] 表示返回值是字典，键是字符串，值可以是任意类型
    """
    # ===== 步骤1：现金流计算 =====
    client_cash = client.get_client_cash_inflow()
    total_trade_cash = sum(h.get_trade_cash_outflow() for h in hedge_list)  # sum(生成器)：求和
    net_day1_cash = client_cash - total_trade_cash  # 净现金 = 客户端流入 - 对冲端流出
...
# 【用途】组员在此测试 is_effective_hedge 函数是否能正确判断对冲有效性
# 【场景】券商卖出一笔中证1000看跌期权（名义本金10亿），做多5000万场内期权对冲
# 【预期】由于标的代码相同（000852），应返回 True
# ================================================================

client_product = ClientContract(
    contract_type = "put_option",       # 看跌期权
    direction = "short",                # 券商卖出
    notional = 100000,                  # 名义本金 100,000万元 = 10亿
    underlying_type = "Index",          # 标的类型：指数（注意与预期值"index_component"不一致）
    underlying_name = "中证1000",
    underlying_code = "000852"          # 中证1000指数代码
    )
hedge_list = [HedgeTrade(tool_type = "onsite_option",   # 场内期权
                         direction = "long",             # 做多
                         notional = 50000,               # 名义价值 5亿
                         underlying_type = "Index",
                         underlying_name = "中证1000",
                         underlying_code = "000852",     # 同一标的
                         option_delta = 0.95)]           # Delta=95%
is_effective = is_effective_hedge(client_product, hedge_list)
print(is_effective)  # 在终端打印结果：True/False