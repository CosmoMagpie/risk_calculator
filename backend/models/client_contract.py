...
class OtcContract:
    ...
    is_equity: Optional[bool] = None       # 是否为权益类业务，即底层资产是否属于权益类资产
    ...
    contract_multiplier: Optional[int] = None  # 合约乘数（万元/点），类似股指期权每点对应多少元
    ...
    equity_swap_margin_amount: Optional[float] = None       # 保证金绝对值（万元），与margin_rate二选一
    ...
    def get_otc_is_equity(self) -> Optional[bool]:
        """获取标的资产是否为权益类资产"""
    ...
    def get_otc_delta_amount(self) -> float:
        ...
        if "option" in self.contract_type:
            is_call = 1 if self.option_type == "call" else -1  # 看涨=+1, 看跌=-1
