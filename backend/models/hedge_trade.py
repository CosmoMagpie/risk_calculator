...
class HedgeTrade:
    ...
    tool_type: str              # 工具类型字符串：
                                ...
                                # "etf" | "other_index_fund" | "other_equity_fund" | "monetary_fund" | "interest_rate_bond_index_fund" | "other_non_equity_fund"
    ...
    is_equity: Optional[bool] = None       # 是否为权益类工具
    ...
    def get_trade_cash_outflow(self) -> float:
        ...
        # --- 场内期权：买入付权利金(+) / 卖出收权利金(-) ---
            ...
            if self.total_premium_amount is not None:
                return self.total_premium_amount if self.direction == "long" else -self.total_premium_amount
    ...
    def get_tool_is_equity(self) -> bool:
        """获取对冲工具是否为权益类工具"""
        return self.is_equity
    ...
    def get_hedge_delta_amount(self) -> Optional[float]:
        ...
        # --- 期货Delta ---
        elif "future" in self.tool_type:
            return self.notional * is_long
