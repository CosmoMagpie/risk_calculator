...
from .risk_reserve import otc_option_short_investment_size
...
def _calc_hedge_rsf(hedge_list: List[HedgeTrade]) -> float:
    ...
    for h in hedge_list:
        ...
        # --- 一揽子股票 ---
        elif h.tool_type in ("index_component", "general_stock", "restricted_stock", "other_stock"):
            st = h.tool_type
            rsf += h.notional * RSF_RATES.get(st, RSF_RATES["general_stock"])

        # --- 场内期货（名义价值 × 12%）---
        elif h.tool_type in ("stock_index_future", "treasury_future", "bulk_commodity_future", "stock_index_future"):
        ...
        # --- 场外背对背对冲（按互换 1% 计提）---
        elif h.tool_type in ("equity_swap", "interest_rate_swap", "otc_option"):
            rsf += h.notional * RSF_RATES["equity_swap"]

        # --- 私募基金 ---
        elif h.tool_type == "single_product(private fund)":
...
def _calc_option_nsfr(otc: OtcContract, hedge_list: List[HedgeTrade]) -> Dict[str, float]:
    ...
    if otc.direction == "short":
        s_short = otc_option_short_investment_size(
            ...
            market=otc.get_otc_underlying_market() or "CN"
        )
...
def _calc_income_cert_nsfr(otc: OtcContract, hedge_list: List[HedgeTrade]) -> Dict[str, float]:
    ...
    is_floating = otc.get_is_float_income()
    days = otc.term_days
