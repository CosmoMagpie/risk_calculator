...
def _calc_otc_option_market_risk_reserve(otc: OtcContract, is_effective_hedge: bool) -> float:
    ...
    if otc.get_premium_rate() is not None:
        option_premium = otc.get_premium_rate() * option_notional
    elif otc.get_total_premium_amount() is not None:
        option_premium = otc.get_total_premium_amount()
    else:
        otc.get_single_premium_amount() * otc.get_contract_size()
...
def calc_market_risk_reserve(otc: OtcContract, hedge_list: List[HedgeTrade], is_hedge_effective: bool) -> float:
    ...
    if "option" in otc.get_otc_contract_type():
        otc_reserve += _calc_otc_option_market_risk_reserve(otc, is_hedge_effective)
