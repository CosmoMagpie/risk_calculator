...
def calc_nc_impact(otc: OtcContract, hedge_list: List[HedgeTrade]) -> float:
    ...
    for h in hedge_list:
        ...
        if h.tool_type == "onsite_option" and h.direction == "short":
            if h.option_margin_amount is not None:
                nc_impact -= h.option_margin_amount
    ...
    # === 客户端：卖出场外期权保证金扣减 ===
    if otc.contract_type == "option" and otc.direction == "short":
        if otc.option_margin_amount is not None:
            ...
            nc_impact -= DEFAULT_CONFIG["client"]["otc_option_margin_rate"] * otc.notional
