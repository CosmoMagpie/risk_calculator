...
def _collect_deltas(otc: OtcContract, hedge_list: List[HedgeTrade]) -> Optional[List[float]]:
    """
    ...
    for hedge in hedge_list:
        if hedge.get_tool_type() == "onsite_option":
            ...
            hedge_delta = get_onsite_option_greeks(hedge.get_tool_code(), "delta") * otc.get_otc_contract_notional()
...
def is_effective_hedge(otc: OtcContract, hedge_list: List[HedgeTrade], price_type: str = "close") -> List:
    """
    ...
    【监管效果】达成有效对冲后，市场风险资本准备计提比例从 30%/100% 降至 5%或1%
    ...
    """
    if otc.get_otc_contract_type() == "income_certificate" and otc.get_is_float_income() == False:
        return [False, "固定型收益凭证没有对冲必要"]
    ...
    if not underlying_is_same:
        ...
        otc_prices = otc_price.get(otc_code, [])
        if not otc_prices:
            return [False, f"无法获取产品标的「{otc_code}」的历史价格数据。可能原因：①iFinD登录失败或账户权限不足；②代码格式错误（如159716应为159716.SZ）；③该标的在iFinD数据库中不存在。请检查代码后重试，或联系IT部门确认iFinD服务状态。"]

        for hc in hedge_codes:
            h_price = get_underlying_price_series(hc, price_type)
            h_prices = h_price.get(hc, [])
            if not h_prices:
                return [False, f"无法获取对冲工具标的「{hc}」的历史价格数据。可能原因：①iFinD登录失败或账户权限不足；②代码格式错误；③该标的在iFinD数据库中不存在。请检查代码后重试，或联系IT部门确认iFinD服务状态。"]

            corr = calculate_correlation_between_price_series(
                otc_code, otc_price, hc, h_price
            )
            if np.isnan(corr) or corr < 0.95:
                return [False, f"标的 {otc_code} 与 {hc} 历史价格相关性 {corr:.2%}，未达到 95% 的有效对冲标准"]
    ...
    if long_delta_sum == 0:
        return [False, "多头 Delta 金额为零，不满足有效对冲条件"]
    if short_delta_sum == 0:
        return [False, "空头 Delta 金额为零，不满足有效对冲条件"]
