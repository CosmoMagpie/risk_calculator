...
# ===================== iFinD 默认登录账号 =====================
# 【请在此填写】公司统一的 iFinD 默认账号和密码。
# 前端登录面板会默认填入该账号、密码以掩码（********）显示；
# 员工如需使用个人账号，可删除默认内容后自行输入。
# 注意：此处涉及明文密码，请妥善保管本文件，勿提交到公共仓库。
IFIND_DEFAULT_LOGIN = {
    "username": "glzq703",   # ← 在此填写公司默认 iFinD 账号
    "password": "96998XuY",   # ← 在此填写公司默认 iFinD 密码
}
